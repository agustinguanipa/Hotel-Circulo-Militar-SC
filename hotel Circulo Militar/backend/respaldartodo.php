...
/* Conexion y eso*/
/*conexion = mysqli_connect($host, $usurio, $passwd) or die("No se conectar con el servidor MySQL: ".mysql_error());
mysqli_select_db('yami tenshi no fansub', $conexion) or die("No se pudo seleccionar la Base de Datos: ". mysql_error());*/
...
if ( empty($tablas) ) {
    ...
    $respuesta = mysqli_query($cone, $consulta) or die ("No se pudo ejecutar la consulta: ".mysqli_error());
...
$info['mysqlver'] = mysqli_get_server_info($cone);
...
foreach ($tablas as $tabla) {
    ...
    $respuesta = mysqli_query($cone, $consulta)
    ...
    while ($fila = mysqli_fetch_array($respuesta, MYSQLI_ASSOC)) {
            ...
            foreach ($columnas as $columna) {
                if ( gettype($fila[$columna]) == "NULL" ) {
                    ...
                    $values[] = "'".mysqli_real_escape_string($cone, $fila[$columna])."'";
                }
            }
