	...
	if(isset($_POST["enviar"])){
		...
		$vali=mysqli_query($cone,$sql);
		 ...
		 while ($ex=mysqli_fetch_array($vali)) {
             ...
             //aqui busca al Recepcionista
		 	elseif ($ex['fky_tiu']=="u02") {
		 		 if($NUM>0){
				 	session_start();
				 	echo "<script>alert('Bienvenido a Departamento de Recepción')</script>";
				 

				 	$_SESSION['nivel']=2;
				     $_SESSION['cedula']=$ex['fky_per'];
				    $_SESSION['nombreusuario']=$ex['nom_usu'];
				    echo "<script>location.href='../fronentd/recep/prinrecep.php';</script>";
