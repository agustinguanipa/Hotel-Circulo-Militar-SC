...
<div class="container" >
  <table class="table table-bordered table-striped table-active"  style="margin-top: 70px;">
  	<tr><!--la del titulo-->
        <td colspan="12" align="center" style="background-color: #4f99fe !important;" class="table-borderless bg-dark "><h4><b class="text-white">Total a pagar en la habitacion <?php echo $_REQUEST["cod"]; ?></b></h4></td>
      </tr>
  ...
  $w=$_REQUEST['cod'];
          ...
          $tus=mysqli_query($cone,$sql);
           ...
           while ($rh=mysqli_fetch_array($tu)) {
             $day="SELECT days FROM `ocupada` WHERE id='$ido[id]'";
             $exday=mysqli_query($cone,$day);
             while ($d=mysqli_fetch_array($exday)) {
             $da=intval($d[0]);
             $r=intval($rusu[0]);
             $h=intval($rh[0]);
             $s=($r+$h)*$da;
             $achu="INSERT INTO `chucuta` (`ocupacion`, `total`, `fecha`) VALUES ('$ido[id]', '$s', NOW());";
